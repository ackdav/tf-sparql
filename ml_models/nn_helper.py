...
def no_modell_mean_error(y_vals0, y_vals1):
    '''calculates the relative mean error without any model
    rel_error = abs(val - mean) / mean
    returns avg(rel_error) over training and dataset
    '''
    mean_ = y_vals0.mean()
    mean_sum = 0.
    for y_ in y_vals0:
        ...
        mean_sum += mean_error
    y_res0 = float(mean_sum.sum() / y_vals0.shape[0])
    ...
    for y_ in y_vals1:
        mean_error = abs(y_- mean_) / mean_
        mean_sum += mean_error
    y_res1 = float(mean_sum.sum() / y_vals1.shape[0])

    return y_res0, y_res1
